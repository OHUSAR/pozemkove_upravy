...
function getArticleList($type) {
    ...
    $sqlQuery = "SELECT * FROM `articles` WHERE type=".$type." ORDER BY date_published DESC LIMIT 5 OFFSET ".$offset.";";
    $qResAndLink = dbConnectAndExecQuerry($sqlQuery);
    $result = $qResAndLink[0];
    ...
    <div class="col-lg-12 text-center">
        <ul class="pager">
            <?php
            $sqlQuery = "SELECT * FROM `articles` WHERE type=".$type." ORDER BY date_published DESC LIMIT 5 OFFSET ".($offset + 5).";";
            $qResAndLink = dbConnectAndExecQuerry($sqlQuery);
            $result = $qResAndLink[0];
